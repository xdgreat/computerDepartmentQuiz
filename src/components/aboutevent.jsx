import React from "react";
import "../styles/aboutevent.css";

function Aboutevent() {
  return (
    <section className="about-event-container">
      <h1 className="about-event-title">Computer Department Week Outline</h1>
      <div className="about-event-section">
        <div className="about-event-content">
          <h1 className="about-title">Lorem ipsum dolor sit.</h1>
          <p className="about-paragraph">
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Nihil quam
            nisi voluptates repudiandae praesentium illum quasi doloribus cumque
            facere cupiditate eum ullam officiis temporibus dolor iure ratione
            amet nam consectetur, ducimus odit nostrum possimus quibusdam
            laborum? Nisi quasi illo quas doloribus nemo adipisci quos,
            reiciendis ut vero placeat ratione, sint facere obcaecati sequi?
            Laboriosam harum earum aspernatur quod quasi illo autem? Eveniet
            dolorum temporibus cumque, consectetur voluptatibus commodi libero
            neque et. Nesciunt itaque dolorem modi id minus? Placeat, velit hic!
          </p>
          <h1 className="about-title">Lorem ipsum dolor sit.</h1>
          <p className="about-paragraph">
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Nihil quam
            nisi voluptates repudiandae praesentium illum quasi doloribus cumque
            facere cupiditate eum ullam officiis temporibus dolor iure ratione
            amet nam consectetur, ducimus odit nostrum possimus quibusdam
            laborum? Nisi quasi illo quas doloribus nemo adipisci quos,
            reiciendis ut vero placeat ratione, sint facere obcaecati sequi?
            Laboriosam harum earum aspernatur quod quasi illo autem? Eveniet
            dolorum temporibus cumque, consectetur voluptatibus commodi libero
            neque et. Nesciunt itaque dolorem modi id minus? Placeat, velit hic!
          </p>
          <h1 className="about-title">Lorem ipsum dolor sit.</h1>
          <p className="about-paragraph">
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Nihil quam
            nisi voluptates repudiandae praesentium illum quasi doloribus cumque
            facere cupiditate eum ullam officiis temporibus dolor iure ratione
            amet nam consectetur, ducimus odit nostrum possimus quibusdam
            laborum? Nisi quasi illo quas doloribus nemo adipisci quos,
            reiciendis ut vero placeat ratione, sint facere obcaecati sequi?
            Laboriosam harum earum aspernatur quod quasi illo autem? Eveniet
            dolorum temporibus cumque, consectetur voluptatibus commodi libero
            neque et. Nesciunt itaque dolorem modi id minus? Placeat, velit hic!
          </p>
        </div>
      </div>
    </section>
  );
}

export default Aboutevent;
